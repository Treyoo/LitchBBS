...
public interface CommentDAO {
    ...
    String INSERT_FIELDS = " entity_type,entity_id,user_id,content,created_time,status,target_id ";
    String SELECT_FIELDS = " id," + INSERT_FIELDS;

    @Insert({"insert into", TABLE_NAME, "(", INSERT_FIELDS,
            ") values(#{entityType},#{entityId},#{userId},#{content},#{createdTime},#{status},#{targetId})"})
    int addComment(Comment comment);

    @Select({"select", SELECT_FIELDS, "from", TABLE_NAME, "where entity_type=#{entityType} and ",
            "entity_id=#{entityId} and status=0 order by created_time desc limit #{offset},#{limit}"})
